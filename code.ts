...
if (figma.currentPage.children.length > 0) {
  ...
  let frames:FrameNode[] = topLevelFrames();
  
  //sort frames
  let sortedFrames:FrameNode[] = sortFramesByPosition(frames);
...
function connecter(slides:FrameNode[]) {
...
function sortFramesByPosition(frames:FrameNode[]) {
...
function topLevelFrames(page: PageNode = figma.currentPage) {
...
function reactionGenerator(destinationId:string) {
  return {
    "action": {
      "type": "NODE",
